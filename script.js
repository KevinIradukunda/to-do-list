...
document.querySelector('#submit').addEventListener('click', () => {
  ...
  const todo_timestamp_el = document.createElement('span');
  todo_timestamp_el.classList.add('timestamp');
  todo_timestamp_el.innerText = `Added on: ${formattedTimestamp}`;

  todo_content_el.appendChild(todo_input_el);
  todo_content_el.appendChild(todo_timestamp_el);
  ...
  // Add the timestamp as a data attribute for sorting purposes
  todo_el.setAttribute('data-timestamp', timestamp.getTime());

  // Add the todo-item to lists
  const todoList = document.querySelector('.todo-lists');
  todoList.appendChild(todo_el);

  // Sort the todo items based on the timestamp
  const todoItems = Array.from(todoList.querySelectorAll('.todo-item'));
  todoItems.sort((a, b) => a.getAttribute('data-timestamp') - b.getAttribute('data-timestamp'));
